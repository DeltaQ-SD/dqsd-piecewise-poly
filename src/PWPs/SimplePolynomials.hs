...
mulPolys as bs = foldr addPolys zeroPoly (intermediateSums as bs)
    where
        ...
        intermediateSums (Poly (x:xs)) ys =  scale x ys : intermediateSums (Poly xs) (shiftPolyUp ys)
...
convolvePolys (lf, uf, Poly fs) (lg, ug, Poly gs) 
    ...
    | otherwise = -- we know g is narrower than f 
        let
            ...
            sumSeries k mulFactor poly = foldr (\n acc -> acc `plus` (mulFactor n `scale` poly n)) zero [0..k]
            ...
            -}
            makeTerm f = foldr plus zero [(a*b) `scale` convolemakeMonomials m n f | (m,a) <- zip [0..] fs, (n,b) <- zip [0..] gs]
